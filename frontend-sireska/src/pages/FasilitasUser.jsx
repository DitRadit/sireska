...
const FasilitasUser = () => {
  ...
  // Ambil info user dari localStorage untuk cek apakah guest
  ...
  const filteredFasilitas = fasilitas.filter((item) => {
    ...
    if (activeFilter === "Tersedia Sekarang") matchFilter = item.status === "aktif";
    else if (activeFilter === "Indoor")       matchFilter = item.tipe?.toLowerCase() === "indoor";
    else if (activeFilter === "Outdoor")      matchFilter = item.tipe?.toLowerCase() === "outdoor";
        ...
        <main className="flex-grow w-full max-w-[1200px] mx-auto pt-[100px] pb-10 flex flex-col lg:flex-row gap-6 relative">
          ...
          <div className="w-full lg:w-[55%] flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex flex-col gap-4">
                <div>
                  <h1 className="text-2xl lg:text-3xl font-extrabold text-gray-800 mb-1">Fasilitas Kampus</h1>
                  <p className="text-gray-500 text-xs lg:text-sm">
                    Ditemukan <span className="font-bold text-orange-500">{filteredFasilitas.length}</span> fasilitas olahraga & ruangan.
                  </p>

                  {/* Info banner untuk guest */}
                  {isGuest && (
                    <div className="mt-3 flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-4 py-2.5">
                      <span className="material-symbols-outlined text-orange-400 text-[18px]">info</span>
                      <p className="text-[11px] text-orange-700 font-medium">
                        Anda terdaftar sebagai <strong>tamu</strong>. Harga sewa ditampilkan dan pembayaran diproses via QRIS setelah reservasi disetujui.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  {filteredFasilitas.map((item) => (
                    <div key={item.fasilitas_id} className="bg-white rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-1 border border-gray-100 overflow-hidden flex flex-col transition-all duration-300 group">
                      ...
                      <div className="relative h-32 bg-gray-100 overflow-hidden shrink-0">
                        <img
                          src={item.gambar_url || "https://placehold.co/400x300/f8f9fa/a1a1aa?text=No+Image"}
                          alt={item.nama_fasilitas}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        />
                        <div className="absolute top-2 left-2">
                          <span className={`px-2 py-0.5 rounded-md text-[9px] font-black shadow-sm backdrop-blur-md uppercase tracking-wider ${item.status === "aktif" ? "bg-white/95 text-green-600" : "bg-white/95 text-red-500"}`}>
                            {item.status === "aktif" ? "Tersedia" : item.status}
                          </span>
                        </div>

                        {/* Badge harga — hanya tampil untuk guest DAN ada harga */}
                        {isGuest && item.harga_per_jam && (
                          <div className="absolute top-2 right-2">
                            <span className="bg-orange-500 text-white px-2 py-0.5 rounded-md text-[9px] font-black shadow-sm">
          ...
          <div className="hidden lg:block lg:w-[45%]">
            <div className="sticky top-[100px] h-[calc(100vh-140px)] min-h-[450px] rounded-2xl overflow-hidden shadow-sm border border-gray-200 relative">
              ...
              <MapContainer center={[-6.973007, 107.630985]} zoom={16} className="w-full h-full" scrollWheelZoom={true}>
                <TileLayer
                ...
                {filteredFasilitas
                  ...
                  .map((item) => (
                    <Marker key={item.fasilitas_id} position={[item.latitude, item.longitude]} icon={OrangeIcon}>
                      <Popup maxWidth={200}>
                        <div style={{ width: "180px", fontFamily: "Poppins, sans-serif" }}>
                          ...
                          />
                          <p style={{ fontWeight: "700", fontSize: "12px", color: "#1f2937", marginBottom: "2px" }}>{item.nama_fasilitas}</p>
                          <p style={{ fontSize: "10px", color: "#9ca3af", marginBottom: "6px" }}>{item.alamat || item.lokasi || "-"}</p>

                          {/* Harga di popup peta — hanya untuk guest */}
                          {isGuest && item.harga_per_jam && (
                            <p style={{ fontSize: "11px", color: "#f97316", fontWeight: "700", marginBottom: "6px" }}>
                              {formatRupiah(item.harga_per_jam)}/jam
                            </p>
                          )}
                          ...
                          <div style={{ display: "flex", gap: "6px" }}>
                            <button
                              onClick={() => navigate(`/fasilitas/${item.fasilitas_id}`)}
                              style={{ flex: 1, backgroundColor: "#fff", color: "#374151", fontSize: "10px", fontWeight: "700", padding: "6px", borderRadius: "8px", border: "1px solid #e5e7eb", cursor: "pointer" }}
                            >
                              Detail
                            </button>
                            <button
                              onClick={() => navigate(`/booking/${item.fasilitas_id}`)}
                              style={{ flex: 1, backgroundColor: "#f97316", color: "white", fontSize: "10px", fontWeight: "700", padding: "6px", borderRadius: "8px", border: "none", cursor: "pointer" }}
                            >
                              Pesan
                            </button>
                          </div>
